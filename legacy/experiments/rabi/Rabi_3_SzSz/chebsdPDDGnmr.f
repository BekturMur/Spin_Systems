...
c BEGIN
     ...
     +    psiR,psiI,dpsiR,dpsiI)
        ...
        xx=1.0D0
        do i=1,Ltot
          call addHSz(xx,psiR,psiI,i,nstates,pzR,pzI)
        enddo
        timeGlz=0.0D0
c END
        do ifile=1,maxLen
...
c            close(chlog)
          ...
          echoXi=xnI - xnR
c          vecResOut(iptVecResOut+2)=echoXr
c          vecResOut(iptVecResOut+3)=echoXi
          ...
          xnI = ddot(nstates,phiI,1,pzR,1)
          echoYi=xnI - xnR
c          vecResOut(iptVecResOut+4)=echoYr
c          vecResOut(iptVecResOut+5)=echoYi
...
c Calculation mean value of S_{z} (S_{z}S_{z})
          ...
          xnI = ddot(nstates,phiI,1,pzR,1)
          echoZi=xnI - xnR
          vecResOut(iptVecResOut+2)=echoZr
          vecResOut(iptVecResOut+3)=echoZi

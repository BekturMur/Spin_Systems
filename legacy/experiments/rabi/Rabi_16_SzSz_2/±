...
c            close(chlog)
          ...
          xnI = ddot(nstates,phiI,1,pzR,1)
          echoYi=xnI - xnR
          vecResOut(iptVecResOut+4)=echoYr
          vecResOut(iptVecResOut+5)=echoYi


c Calculation mean value of S_{z} (S_{x}S_{z})
          ...
          xnI = ddot(nstates,phiI,1,pzR,1)
          echoZi=xnI - xnR
c          vecResOut(iptVecResOut+6)=echoZr
c          vecResOut(iptVecResOut+7)=echoZi
...
c now calculate norms, use same notations
          ...
          nrmpsi=xnI + xnR
          vecResOut(iptVecResOut+8)=nrmz
          vecResOut(iptVecResOut+9)=nrmpsi

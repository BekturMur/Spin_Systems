...
c        close(chlog)
        ...
        do i=0,maxStat-1
          ...
          psiI(i) = 0.0D0
        enddo
        #если это в первый раз, то это повлияет на то, как будет обратока информация
        firstime = 1
        timeGl = 0.0 #global time
        #у каждого файла свой счетчик времени, а полное время это timeGl
...
c          execute file
          ...
          Lsys = LsysT(ifile) #размер центральной системы
          Lbath = LbathT(ifile) #размер термостата
          Ltot = LtotT(ifile) #полный размер
          do k=1,Ltot
            Hx(k) = HxT(k,ifile) #локальные поля
            Hy(k) = HyT(k,ifile)
            Hz(k) = HzT(k,ifile)
          enddo
          nconn = nconnT(ifile)
          do k=1,nconn
            asp(k) = aspT(k,ifile)
            bsp(k) = bspT(k,ifile) #перепаковка в линейный массив
            Jx(k) = JxT(k,ifile)
            Jy(k) = JyT(k,ifile)
            Jz(k) = JzT(k,ifile)
          enddo
          iRand = iRandT(ifile) #некоторый аналог нулевой итерации для датчика случайных чисел
          nsteps = nstepsT(ifile)
          tau = tauT(ifile)
          init_state = init_stateT(ifile) #начальная волновая функция
          autonorm = autonormT(ifile) #в мнимом времени надо автонормировать потому
          EpsF = EpsFT(ifile) #эпсилон final
          imflag = imflagT(ifile) #мнимое время
          ispecflag = ispecflagT(ifile) #special propagation
          nooutput = nooutputT(ifile) #если что-то неитересное, то выкидываем
...
#чтоб правильно считывала файлы 0 1 2 1 2 1 2 3

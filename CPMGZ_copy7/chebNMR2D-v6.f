...
c example: test individual vecResOut
                open(75,file='veclog2.out',access='append')
                write(75,'(a)') '#' ! separator from other instances of it
                write(75,'(i10)') itC
                i=1
                do while (i.le.iptVecResOut)
                  write(75,'(f10.5$)') vecResOut(i) ! timeC
c                  write(75,'(i5$)') mrkResOut(i) ! timeC
                  do j=i+1,i+mrkResOut(i)-1
                    write(75,'(f10.5$)') vecResOut(j) 
                  enddo
                  i = i + mrkResOut(i)
                  write(75,'(a)') ' ' 
                  write(75,'(a)') ' ' 
                ...
                open(75,file='somedata.out',access='append')
                do i=1,iptVecResOut,4
                  write(75,'(e18.8$)') vecResOut(i)   ! timeGl
                  write(75,'(e18.8$)') vecResOut(i+1) ! sx(1)
                  write(75,'(e18.8$)') vecResOut(i+2) ! sy(1)
                  write(75,'(e18.8)') vecResOut(i+3)  ! sz(1)
                enddo
                close(75)
c end of example
...
c
     ...
     +          MPI_COMM_WORLD,ierr)
          ...
          if (ioutGen.ne.0) then
            ...
            open(71,file='echo2Dbezsqrt.out')
            i=1
            do while (i.le.iptVecResOut)
              timeC = vecResOut(i)
...
c                 write(71,'(f18.10$)') dsqrt(vecResDev(j)/Nit)
c                write(71,'(f18.10$)') vecResAv(j)
                write(71,'(f25.15$)') vecResOut(j) ! used for tests, to check the last vecResOut
              enddo
              i = i + mrkResOut(i)

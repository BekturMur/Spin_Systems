       subroutine chspecstepsPDDGnmr(asp0,bsp0,Jx,Jy,Jz,nconn0,Hx,Hy,Hz,
...
c internal variables
        ...
        integer nstates, i,k,iconn,j,jp, l,i0,ip0,ip1
...
c pi-pulse along X-axis
        if (ispecflag.eq.1) then ! pi-pulse along X
          ...
          enddo
          enddo
        else if (ispecflag.eq.1314) then ! pi-X rotation of all spins, incl. factor -I
          do k=1,Ltot
            call actSx(psiR,psiI,k,nstates,phiR,phiI)
            do l=0,nstates-1
              psiR(l)=phiI(l)
              psiI(l)=-phiR(l)
            enddo
          enddo
        else if (ispecflag.eq.2314) then ! pi-Y rotation of all spins, incl. factor -I
          do k=1,Ltot
            call actSy(psiR,psiI,k,nstates,phiR,phiI)
            do l=0,nstates-1
              psiR(l)=phiI(l)
              psiI(l)=-phiR(l)
            enddo
          enddo
        else if (ispecflag.eq.1157) then ! pi/2-X rotation of all spins, exp(-I*pi/2*sigma_X)
          ar=dsqrt(2.0D0)
          do k=1,Ltot
            call actSx(psiR,psiI,k,nstates,phiR,phiI)
            do l=0,nstates-1
              psiR(l)=ar*(psiR(l)+phiI(l))
              psiI(l)=ar*(psiI(l)-phiR(l))
            enddo
          enddo
        else if (ispecflag.eq.2157) then ! pi/2-Y rotation of all spins, exp(-I*pi/2*sigma_Y)
          ar=dsqrt(2.0D0)
          do k=1,Ltot
            call actSy(psiR,psiI,k,nstates,phiR,phiI)
            do l=0,nstates-1
